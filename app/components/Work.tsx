...
const projects: Project[] = [
  {
    id: 1,
    title: "Trailer Teaser | AD for my YouTube post",
    description:
      "Cinematic teaser edit crafted for YouTube, showcasing advanced colour grading and dynamic pacing.",
    platform: "youtube",
  ...
  {
    id: 2,
    title: "Video diary: Nothing is ever perfect anyway🤷🏻‍♀️",
    description:
      "Personal vlog-style edit blending original footage. Colour graded for a warm, cinematic feel with a focus on storytelling.",
    platform: "youtube",
    tools: ["DaVinci Resolve", "CapCut"],
  ...
  {
    id: 7,
    title: "Multipotentialite in the content creation space",
    description:
      "Video essay-style edit exploring the concept of being a multipotentialite. Integrated motion graphics and dynamic cuts to maintain engagement while conveying complex ideas.",
    platform: "youtube",
    tools: ["CapCut"],
  ...
  youtube: {
    ...
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-8 h-8 text-red-500/60"
      >
  ...
  tiktok: {
    ...
    playIcon: (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-12 h-12 text-cyan-500/40"
      >
        <path d="M8 5v14l11-7z" />
      </svg>
    ),
  ...
  instagram: {
    ...
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-8 h-8 text-pink-500/50"
      >
    ...
    playIcon: (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-12 h-12 text-pink-500/40"
      >
        <path d="M8 5v14l11-7z" />
      </svg>
    ),
...
export default function Work() {
  ...
  const totalPages =
    active === "all" ? Math.ceil(filtered.length / ITEMS_PER_PAGE) : 1;
  const displayed =
    active === "all"
      ? filtered.slice(
  ...
  return (
    <section id="work" className="py-24 px-6 sm:px-10 lg:px-16">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <p className="text-xs text-[#c9a53d] tracking-widest uppercase mb-3">
            Selected Work
          </p>
          ...
          {displayed.map((project) => {
            ...
            const instagramEmbed =
              project.platform === "instagram"
                ? getInstagramEmbedUrl(project.link)
                : null;
            const tiktokEmbed =
              project.platform === "tiktok"
                ? getTikTokEmbedUrl(project.link)
                : null;
                ...
                <div className="flex-1 flex flex-col justify-between p-6 md:p-8">
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      <span
                        className={`text-xs px-2.5 py-0.5 rounded-full border ${config.badge}`}
                      >
                        {config.label}
                      </span>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {project.tools.map((tool) => (
                        <span
                          key={tool}
                          className="text-xs px-2 py-0.5 rounded bg-[#1a1a1a] text-[#a0998e]"
                        >
                          {tool}
                        </span>
                      ))}
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {(
            ...
            >
              <div>
                <p className="text-xs text-[#6b6b6b] tracking-wide mb-0.5">
                  {ch.platform}
                </p>
